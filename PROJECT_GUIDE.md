...
下面给出一个高层分层视图与各层职责，便于快速理解系统组件与调用关系。

架构（简化视图）：

```
Presentation (GUI)  <--->  Application (Controllers/Orchestrator)  <--->  Domain (Strategies/Engine)
       |                             |                                         |
       |                             |                                         |
   qbot/gui/*                 qbot_main.py / monitoring.py                 qbot/strategies/*
       |                             |                                         |
       v                             v                                         v
   用户操作 -> 请求回测/交易 -> 调度器/回测引擎 -> 策略计算 -> 交易执行/结果输出
       ^                             |                                         |
       |                             v                                         v
    WebPanel                    Data Layer (qbot/data, utils)        Execution Layer (trade engines, easytrader)
```

分层职责：
- 表现层（Presentation / UI）: `qbot/gui/`，提供可视化回测、策略管理、交易面板、嵌入网页与仪表盘。入口文件：[main.py](main.py)
- 应用层（Application / Orchestrator）: `qbot_main.py`, `monitoring.py`，负责调度实时监控、策略融合、通知与任务调度。
- 领域层（Domain / Strategy）: `qbot/strategies/` 与 `qbot/strategy/`，实现各类交易策略（传统指标、机器学习、RL）。回测接口由 `qbot/engine/backtest/` 提供。
- 数据层（Data）: `qbot/data/`, `utils/common/TuShare.py` 等，封装行情获取、预处理、缓存、导出功能。
- 执行/集成层（Integration / Execution）: `qbot/engine/trade/`, `pytrader/`, `pyfunds/`，封装券商/交易所、模拟/实盘下单逻辑与适配器。
- 插件与分析（Plugins/Analytics）: `qbot/plugins/quantstats` 等，提供绩效分析、监控与扩展能力。

关键交互与典型调用流程：
- 启动流程：运行 `python main.py` -> `qbot.gui.mainframe.MainFrame` 初始化 -> 用户在 GUI 选择回测/策略 -> 前端将参数传递给回测面板 `PanelBacktest` -> 调用回测引擎 `qbot/engine/backtest/backtest_main.py`。
- 回测流程：回测引擎读取数据 (`qbot/data/` 或 `pytrader` 数据接口) -> 为每个时间步调用策略 `next()` -> 下单逻辑由回测框架处理并记录绩效 -> 使用 `qbot/plugins/quantstats` 输出报告。
- 实盘流程：`qbot_main.py`/`monitoring.py` 采集实时行情 -> 多因子融合/策略判断 -> 调用 execution adapter（如 `easytrader`/API）下单 -> 推送通知（邮件/飞书/桌面）。

部署与运行要点：
- GUI 需要 `wxPython`，推荐使用与系统 Python 版本兼容的 wheel（仓库 `dev/` 目录已包含常见 wheel）。
- 实盘需要券商 API 权限和安全配置（请在 `qbot/common/config.py` 中填写凭证信息）。
- 长期运行建议使用 `systemd`/Windows 服务或容器化部署；后台监控可通过 `nohup python qbot/plugins/auto_monitor.py > monitoring.log &` 启动。

监控与日志：
- 日志统一使用 `qbot/common/logging/logger.py`（请根据部署环境调整日志级别与日志轮转）。
- 关键组件应暴露健康检查接口（示例：`monitoring.py` 的 `report()` 用于告警）。
